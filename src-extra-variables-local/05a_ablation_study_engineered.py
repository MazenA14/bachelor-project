...
print("Initiating Phase 5: Ablation Study (Global vs. Local vs. Hybrid)...\n")
...
ENGINEERED_FILE = os.path.join(ROOT_DIR, '03_final', '01a_engineered_differencing_metals_dataset.csv')
...
# We must drop today's absolute prices and today's log returns so the model doesn't cheat.
# We use a safe-drop method in case some variables didn't get log-returned.
potential_leakage = [
    'Gold_Close', 'Silver_Close', 'DXY_Close', 'EGP_USD_Close', 'VIX_Close', 'SP500_Close', 'Local_Gold_24k_EGP',
    'Gold_Close_LogReturn', 'Silver_Close_LogReturn', 'DXY_Close_LogReturn', 
    'EGP_USD_Close_LogReturn', 'VIX_Close_LogReturn', 'SP500_Close_LogReturn', 'Local_Gold_24k_EGP_LogReturn'
]
...
xgb_local = xgb.XGBRegressor(n_estimators=1000, learning_rate=0.05, max_depth=5, random_state=42, early_stopping_rounds=50)
...
plt.title('Ablation Study: Forecasting Error (RMSE) by Feature Set', fontsize=14)
...
plt.title('Local Gold (24K EGP) Forecast: Testing Feature Isolation', fontsize=16)
