...
print("Initiating Phase 4: Model Evaluation (No Extras, With ARIMA Recalculation)...\n")
...
history = list(df_master['Local_Gold_24k_EGP'][:TRAIN_VAL_END])
# We grab the actual answers for the test set
test_actuals = df_master['Local_Gold_24k_EGP'][TEST_START:]
...
yesterday_price_a = df_master.loc[X_test_a.index, 'Local_Gold_24k_EGP'].shift(1)
yesterday_price_a.iloc[0] = df_master.loc[TRAIN_VAL_END, 'Local_Gold_24k_EGP']
...
trend_expected_b = df_b.loc[X_test_b.index, 'Local_Gold_24k_EGP_Trend']
xgb_b_price_predictions = trend_expected_b + preds_resid_b

# --- 6. CALCULATE FINAL SCORES (RMSE) ---
actual_prices = df_master.loc[test_a.index, 'Local_Gold_24k_EGP']
...
plt.title('Local Gold (24K EGP) Forecast: No Extras - 1-Day Rolling (Test Set)', fontsize=16)
