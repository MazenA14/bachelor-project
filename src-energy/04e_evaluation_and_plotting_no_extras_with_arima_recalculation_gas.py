...
print("Initiating Phase 4: Model Evaluation — Natural Gas (No Extras, With ARIMA Walk-Forward Recalculation)...\n")
...
xgb_a_full    = xgb.XGBRegressor(); xgb_a_full.load_model(os.path.join(MODELS_DIR, 'xgboost_a_gas.json'))
xgb_b_full    = xgb.XGBRegressor(); xgb_b_full.load_model(os.path.join(MODELS_DIR, 'xgboost_b_gas.json'))
xgb_a_noextra = xgb.XGBRegressor(); xgb_a_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_a_no_extras_gas.json'))
xgb_b_noextra = xgb.XGBRegressor(); xgb_b_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_b_no_extras_gas.json'))

# --- 3. ARIMA BASELINE: ONE-STEP-AHEAD ROLLING FORECAST ---
print("\nGenerating ARIMA Walk-Forward Forecast (this takes a few minutes)...")
history      = list(df_master['Natural_Gas_Close'][:TRAIN_VAL_END])
test_actuals = df_master['Natural_Gas_Close'][TEST_START:]
arima_preds  = []
...
for t in range(len(test_actuals)):
    ...
    if (t + 1) % 50 == 0:
        print(f"  ARIMA Progress: {t + 1} / {len(test_actuals)} days...")

arima_predictions = pd.Series(arima_preds, index=test_actuals.index)
print("ARIMA Walk-Forward Forecast complete.\n")

# --- 4. XGBOOST PREDICTIONS & PRICE REVERSAL ---
print("Generating XGBoost predictions...")
actual_prices    = df_master.loc[test_a_noextra.index, 'Natural_Gas_Close']
last_train_price = df_master.loc[TRAIN_VAL_END, 'Natural_Gas_Close']


def reverse_log_returns(preds_log, test_index):
    yesterday = df_master.loc[test_index, 'Natural_Gas_Close'].shift(1)
    yesterday.iloc[0] = last_train_price
    return yesterday * np.exp(preds_log)


def reverse_detrend(preds_resid, df_source, test_index):
    return df_source.loc[test_index, 'Natural_Gas_Close_Trend'] + preds_resid
...
print("\n=== EVALUATION RESULTS — NATURAL GAS (Test Set: January 2025 – Present) ===")
print(f"ARIMA Walk-Forward Baseline:                   RMSE=${m_arima['rmse']:.2f}  MAPE={m_arima['mape']:.4f}  MAE={m_arima['mae']:.2f}  R²={m_arima['r2']:.4f}")
...
fig.suptitle('Natural Gas Price Forecast — Test Period (January 2025 – Present)\nARIMA: Walk-Forward Recalculation', fontsize=14, fontweight='bold')
...
for p in panels:
    ...
    ax.plot(arima_predictions.index,  arima_predictions,  label=f'ARIMA Walk-Forward Prediction (RMSE: ${m_arima["rmse"]:.2f})',
            color='darkorange',  linewidth=1.2, linestyle=':')
    ax.set_title(p['title'], fontsize=11, pad=10)
    ax.set_ylabel('Natural Gas Price (USD/MMBtu)', fontsize=10)
...
fig2.suptitle('Top 10 Most Important Features by Model Variant — Natural Gas', fontsize=14, fontweight='bold')
