...
#   Stationarity Method : DIFFERENCING  — reads 01a_engineered_differencing_energy_dataset.csv
...
print("Initiating Phase 5: Ablation Study (Energy - Engineered Features)...\n")
print("Research Question: Does adding local Egyptian economic data improve Brent forecasting?\n")

# --- 1. DIRECTORY SETUP ---
ROOT_DIR = '../data-energy/'
MASTER_FILE = os.path.join(ROOT_DIR, '02_processed', '01_master_energy_dataset.csv')
ENGINEERED_FILE = os.path.join(ROOT_DIR, '03_final', '01a_engineered_differencing_energy_dataset.csv')

# --- 2. DATA LOADING & TIMELINE SPLIT ---
df_master = pd.read_csv(MASTER_FILE, index_col='Date', parse_dates=['Date'])
df_eng = pd.read_csv(ENGINEERED_FILE, index_col='Date', parse_dates=['Date'])
...
train = df_eng[:TRAIN_END]
val = df_eng[VAL_START:VAL_END]
...
# The Hybrid Dataset has EVERYTHING (global + local) — target is Brent Crude log return
y_train = train['Brent_Crude_Close_LogReturn']
X_train_hybrid = train.drop(columns=cols_to_drop)

y_val = val['Brent_Crude_Close_LogReturn']
X_val_hybrid = val.drop(columns=cols_to_drop)

y_test = test['Brent_Crude_Close_LogReturn']
...
# (energy: Brent lags/rolling + NatGas + Ratio, AND macro: DXY, VIX, SP500, US_10Yr)
...
yesterday_price = df_master.loc[X_test_hybrid.index, 'Brent_Crude_Close'].shift(1)
yesterday_price.iloc[0] = df_master.loc['2024-12-31', 'Brent_Crude_Close']  # Fill first day
...
plt.title('Ablation Study: Impact of Local Egyptian Data on Brent Forecast (Engineered)', fontsize=13)
plt.ylabel('RMSE in USD ($/bbl)', fontsize=12)
...
plt.title('Brent Crude Forecast: Global Only vs Hybrid (Engineered Features)', fontsize=16)
