...
print("Initiating Phase 5d: Ablation Study (Cross-Energy Isolation, Global Only)...\n")
...
# =====================================================================
print("=" * 60)
print("  PART A: BRENT CRUDE PRICE PREDICTION (Global Only)")
print("=" * 60)
...
cols_to_drop_brent_full = [c for c in drop_cols_brent_full if c in train.columns]
...
# =====================================================================
print("=" * 60)
print("  PART B: NATURAL GAS PRICE PREDICTION (Global Only)")
print("=" * 60)
...
cols_to_drop_natgas_full = [c for c in drop_cols_natgas_full if c in train.columns]
...
plt.suptitle('Ablation Study: Cross-Energy Feature Contribution (Global Only)', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.title('Brent Crude Forecast: With vs. Without NatGas Features (Global Only)', fontsize=16)
...
plt.title('Natural Gas Forecast: With vs. Without Brent Features (Global Only)', fontsize=16)
plt.ylabel('Natural Gas Price (USD/MMBtu)', fontsize=12)
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
plt.show()

print("\n--- CROSS-ENERGY ABLATION STUDY (GLOBAL ONLY) COMPLETE ---")
