...
#   Saves models to     : ../models-energy/  (xgboost_a_no_extras_gas.json, xgboost_b_no_extras_gas.json,
#                                             arima_baseline_no_extras_gas.pkl)
...
print("Initiating Phase 3: Model Training — Natural Gas (No Extras)...\n")
...
arima_save_path = os.path.join(MODELS_DIR, 'arima_baseline_no_extras_gas.pkl')
arima_fitted.save(arima_save_path)
print(f"ARIMA Base saved to: {arima_save_path}")

# --- 6. MODEL 2: XGBOOST ARCHITECTURE A ---
print("\nTraining Model 2: XGBoost (Log Returns, No Extras, Natural Gas)...")
...
xgb_a_save_path = os.path.join(MODELS_DIR, 'xgboost_a_no_extras_gas.json')
xgb_a.save_model(xgb_a_save_path)
print(f"XGBoost A saved to: {xgb_a_save_path}")

# --- 7. MODEL 3: XGBOOST ARCHITECTURE B ---
print("\nTraining Model 3: XGBoost (Detrended Residuals, No Extras, Natural Gas)...")
...
xgb_b_save_path = os.path.join(MODELS_DIR, 'xgboost_b_no_extras_gas.json')
xgb_b.save_model(xgb_b_save_path)
print(f"XGBoost B saved to: {xgb_b_save_path}")

print("\n--- TRAINING PHASE (NO EXTRAS, NATURAL GAS) COMPLETE ---")
