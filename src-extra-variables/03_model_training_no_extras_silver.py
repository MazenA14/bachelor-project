...
print("Initiating Phase 3: Model Training Protocol — Silver (No Extras)...\n")
...
y_train_a = train_a['Silver_Close_LogReturn']
X_val_a = val_a.drop(columns=drop_cols_a)
y_val_a = val_a['Silver_Close_LogReturn']
...
y_train_b = train_b['Silver_Close_Residual']
X_val_b = val_b.drop(columns=drop_cols_b)
y_val_b = val_b['Silver_Close_Residual']
...
print("Training Model 1: ARIMA Baseline (Silver)...")
arima_train = df_master['Silver_Close'][:VAL_END]
arima_model = ARIMA(arima_train, order=(5, 1, 0))
arima_fitted = arima_model.fit()

arima_save_path = os.path.join(MODELS_DIR, 'arima_baseline_no_extras_silver.pkl')
arima_fitted.save(arima_save_path)
print(f"ARIMA Base saved to: {arima_save_path}")

# --- 6. MODEL 2: XGBOOST ARCHITECTURE A ---
print("\nTraining Model 2: XGBoost (Log Returns, No Extras, Silver)...")
...
xgb_a_save_path = os.path.join(MODELS_DIR, 'xgboost_a_no_extras_silver.json')
xgb_a.save_model(xgb_a_save_path)
print(f"XGBoost A saved to: {xgb_a_save_path}")

# --- 7. MODEL 3: XGBOOST ARCHITECTURE B ---
print("\nTraining Model 3: XGBoost (Detrended Residuals, No Extras, Silver)...")
...
xgb_b_save_path = os.path.join(MODELS_DIR, 'xgboost_b_no_extras_silver.json')
xgb_b.save_model(xgb_b_save_path)
print(f"XGBoost B saved to: {xgb_b_save_path}")

print("\n--- TRAINING PHASE (NO EXTRAS, SILVER) COMPLETE ---")
