...
print("Initiating Phase 4: Model Evaluation — Silver (No Extras, With ARIMA Walk-Forward Recalculation)...\n")
...
xgb_a_full    = xgb.XGBRegressor(); xgb_a_full.load_model(os.path.join(MODELS_DIR, 'xgboost_a_silver.json'))
xgb_b_full    = xgb.XGBRegressor(); xgb_b_full.load_model(os.path.join(MODELS_DIR, 'xgboost_b_silver.json'))
xgb_a_noextra = xgb.XGBRegressor(); xgb_a_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_a_no_extras_silver.json'))
xgb_b_noextra = xgb.XGBRegressor(); xgb_b_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_b_no_extras_silver.json'))

# --- 3. ARIMA BASELINE: ONE-STEP-AHEAD ROLLING FORECAST ---
print("\nGenerating ARIMA Walk-Forward Forecast (this takes a few minutes)...")
history      = list(df_master['Silver_Close'][:TRAIN_VAL_END])
test_actuals = df_master['Silver_Close'][TEST_START:]
arima_preds  = []
...
actual_prices    = df_master.loc[test_a_noextra.index, 'Silver_Close']
last_train_price = df_master.loc[TRAIN_VAL_END, 'Silver_Close']


def reverse_log_returns(preds_log, test_index):
    yesterday = df_master.loc[test_index, 'Silver_Close'].shift(1)
    yesterday.iloc[0] = last_train_price
    return yesterday * np.exp(preds_log)


def reverse_detrend(preds_resid, df_source, test_index):
    return df_source.loc[test_index, 'Silver_Close_Trend'] + preds_resid
...
print("\n=== EVALUATION RESULTS — SILVER (Test Set: January 2025 – Present) ===")
...
fig.suptitle('Silver Price Forecast — Test Period (January 2025 – March 2026)\nARIMA: Walk-Forward Recalculation', fontsize=14, fontweight='bold')
...
for p in panels:
    ...
    ax.set_ylabel('Silver Price (USD)', fontsize=10)
...
fig2.suptitle('Top 10 Most Important Features by Model Variant — Silver', fontsize=14, fontweight='bold')
