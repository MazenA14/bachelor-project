...
def main() -> None:
    ...
    print("=" * 60)
    print("  CEEMDAN-XGBoost — Phase 2c: Sub-model training (stationary)")
    print("=" * 60)

    try:
        df = pd.read_csv(input_file, index_col="Date", parse_dates=["Date"])
        print(f"[OK] Stationary dataset loaded: {len(df)} rows x {len(df.columns)} cols")
    ...
    print("\n" + "-" * 60)
    print("  Training stationary sub-models")
    print("-" * 60)
    ...
    for target in imf_targets:
        ...
        if target == "Residue" and use_ridge_for_residue():
            ...
            path = os.path.join(MODELS_DIR, "ceemdan_stationary_ridge_residue.joblib")
        ...
        out_path = os.path.join(MODELS_DIR, f"ceemdan_stationary_xgb_{target.lower()}.json")
        model.save_model(out_path)
        print(f"  [OK] Saved -> {out_path}")

    print("\n" + "=" * 60)
    print("  STATIONARY CEEMDAN SUB-MODELS COMPLETE")
    print("=" * 60)
