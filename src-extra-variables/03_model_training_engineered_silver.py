...
print("Initiating Phase 3: Model Training Protocol — Silver (Engineered Features)...\n")
...
arima_save_path = os.path.join(MODELS_DIR, 'arima_baseline_silver.pkl')
arima_fitted.save(arima_save_path)
print(f"ARIMA Base saved to: {arima_save_path}")

# --- 6. MODEL 2: XGBOOST ARCHITECTURE A ---
print("\nTraining Model 2: XGBoost (Log Returns, Silver)...")
...
xgb_a_save_path = os.path.join(MODELS_DIR, 'xgboost_a_silver.json')
xgb_a.save_model(xgb_a_save_path)
print(f"XGBoost A saved to: {xgb_a_save_path}")

# --- 7. MODEL 3: XGBOOST ARCHITECTURE B ---
print("\nTraining Model 3: XGBoost (Detrended Residuals, Silver)...")
...
xgb_b_save_path = os.path.join(MODELS_DIR, 'xgboost_b_silver.json')
xgb_b.save_model(xgb_b_save_path)
print(f"XGBoost B saved to: {xgb_b_save_path}")

print("\n--- TRAINING PHASE (ENGINEERED, SILVER) COMPLETE ---")
