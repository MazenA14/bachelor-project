...
print("Initiating Phase 5c: Ablation Study (Single-Metal Isolation) - NO EXTRAS...\n")
...
# =====================================================================
print("=" * 60)
print("  PART A: GOLD PRICE PREDICTION")
print("=" * 60)
...
xgb_gold_only = xgb.XGBRegressor(
    n_estimators=500, learning_rate=0.01, max_depth=3,
    subsample=0.8, colsample_bytree=0.8, random_state=42)
xgb_gold_only.fit(X_train_gold_only, y_train_gold,
...
preds_gold_only = xgb_gold_only.predict(X_test_gold_only)
...
# =====================================================================
print("=" * 60)
print("  PART B: SILVER PRICE PREDICTION")
print("=" * 60)
...
xgb_silver_only = xgb.XGBRegressor(
    n_estimators=500, learning_rate=0.01, max_depth=3,
    subsample=0.8, colsample_bytree=0.8, random_state=42)
xgb_silver_only.fit(X_train_silver_only, y_train_silver,
...
preds_silver_only = xgb_silver_only.predict(X_test_silver_only)
...
plt.suptitle('Ablation Study: Cross-Metal Feature Contribution', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.title('Gold Price Forecast: With vs. Without Silver Features', fontsize=16)
...
plt.title('Silver Price Forecast: With vs. Without Gold Features', fontsize=16)
plt.ylabel('Silver Price (USD)', fontsize=12)
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
plt.show()

print("\n--- SINGLE-METAL ABLATION STUDY COMPLETE ---")
