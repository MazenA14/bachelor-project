# =============================================================================
# PIPELINE IDENTITY
#   Commodity           : Sugar (Crops Pipeline)
...
#   Models loaded from  : ../models-crops/
# =============================================================================
import os
import pandas as pd
import numpy as np
...
print("Initiating Phase 4: Model Evaluation — Sugar (With ARIMA Walk-Forward Recalculation)...\n")
...
xgb_a_full    = xgb.XGBRegressor(); xgb_a_full.load_model(os.path.join(MODELS_DIR, 'xgboost_a_sugar.json'))
xgb_b_full    = xgb.XGBRegressor(); xgb_b_full.load_model(os.path.join(MODELS_DIR, 'xgboost_b_sugar.json'))
xgb_a_noextra = xgb.XGBRegressor(); xgb_a_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_a_no_extras_sugar.json'))
xgb_b_noextra = xgb.XGBRegressor(); xgb_b_noextra.load_model(os.path.join(MODELS_DIR, 'xgboost_b_no_extras_sugar.json'))

# --- 3. ARIMA BASELINE: ONE-STEP-AHEAD ROLLING FORECAST ---
print("\nGenerating ARIMA Walk-Forward Forecast (this takes a few minutes)...")
history      = list(df_master['Sugar_Close'][:TRAIN_VAL_END])
test_actuals = df_master['Sugar_Close'][TEST_START:]
arima_preds  = []
...
actual_prices    = df_master.loc[test_a_full.index, 'Sugar_Close']
last_train_price = df_master.loc[TRAIN_VAL_END, 'Sugar_Close']


def reverse_log_returns(preds_log, test_index):
    yesterday = df_master.loc[test_index, 'Sugar_Close'].shift(1)
    yesterday.iloc[0] = last_train_price
    return yesterday * np.exp(preds_log)


def reverse_detrend(preds_resid, df_source, test_index):
    return df_source.loc[test_index, 'Sugar_Close_Trend'] + preds_resid
...
print("\n=== EVALUATION RESULTS — SUGAR (Test Set: January 2025 – Present) ===")
...
fig.suptitle('Sugar Price Forecast — Test Period (January 2025 – Present)\nARIMA: Walk-Forward Recalculation', fontsize=14, fontweight='bold')
...
for p in panels:
    ...
    ax.set_ylabel('Sugar Price (USD/lb)', fontsize=10)
...
fig2.suptitle('Top 10 Most Important Features by Model Variant — Sugar', fontsize=14, fontweight='bold')
