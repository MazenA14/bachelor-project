...
#   Saves models to     : ../models-crops/  (xgboost_a_corn.json, xgboost_b_corn.json, arima_baseline_corn.pkl)
# =============================================================================
import os
import pandas as pd
...
print("Initiating Phase 3: Model Training Protocol — Corn (Engineered Features)...\n")
...
# Architecture A: Log Returns — drop raw prices + Corn log return (target)
drop_cols_a = [
...
# Architecture B: Detrending — drop raw prices + Corn trend/residual columns (target is Corn_Close_Residual)
drop_cols_b = [
    'Wheat_Close', 'Corn_Close', 'Sugar_Close', 'Brent_Crude_Close', 'DXY_Close', 'EGP_USD_Close',
    'Wheat_Close_Trend',
...
xgb_a_save_path = os.path.join(MODELS_DIR, 'xgboost_a_corn.json')
xgb_a.save_model(xgb_a_save_path)
print(f"XGBoost A saved to: {xgb_a_save_path}")

# --- 7. MODEL 3: XGBOOST ARCHITECTURE B ---
print("\nTraining Model 3: XGBoost (Detrended Residuals, Corn)...")
...
xgb_b_save_path = os.path.join(MODELS_DIR, 'xgboost_b_corn.json')
xgb_b.save_model(xgb_b_save_path)
print(f"XGBoost B saved to: {xgb_b_save_path}")

print("\n--- TRAINING PHASE (ENGINEERED, CORN) COMPLETE ---")
