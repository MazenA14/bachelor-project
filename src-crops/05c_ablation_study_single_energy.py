...
print("Initiating Phase 5c: Ablation Study (Single Crop vs. All Three)...\n")
...
def evaluate_commodity(target_prefix, other_keywords):
    print("=" * 60)
    print(f"  PREDICTING {target_prefix.upper()} PRICE")
    print("=" * 60)
    ...
    cols_to_drop = [c for c in drop_cols if c in df_diff.columns]
...
plt.suptitle('Ablation Study: Single Crop vs. All Three Features', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.show()
...
def plot_ts(actual, price_full, price_single, rmse_full, rmse_single, name, color_full, color_single, unit):
    ...
    plt.title(f'{name} Forecast: With vs. Without Other Crop Features', fontsize=16)
...
print("\n--- SINGLE CROP VS ALL THREE ABLATION STUDY COMPLETE ---")
